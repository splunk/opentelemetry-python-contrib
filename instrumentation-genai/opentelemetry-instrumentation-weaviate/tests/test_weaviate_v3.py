...
class TestWeaviateV3SpanGeneration(WeaviateSpanTestBase):
    ...
    def test_similarity_attribute(self):
        client = self.get_weaviate_client()
        near_text_filter = {
        ...
        (
            client.query.get("Articles", ["text", "author"])
        ...
        spans = self.assert_span_count(1)
        span = spans[0]
        self.assertIn("db.weaviate.similarity", span.attributes)
        self.assertEqual(span.attributes["db.weaviate.similarity"], "cosine")
